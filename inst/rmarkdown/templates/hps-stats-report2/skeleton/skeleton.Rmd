...
<Div custom-style = "Heading 2_nopgbrk">Introduction</div>
...
<Div custom-style = "Heading 2_nopgbrk">Main Points</div>
...
<Div custom-style = "Heading 2_nopgbrk">Results and Commentary</div>
...
## List of Tables
<br>
```{r kable_ListOfTables, echo = FALSE, ft.align="left"}
flextable(ListofTables, theme_fun = NULL) %>% 
  my_ft_format() %>% 
  set_header_labels(file_name = "File name",
                    file_size = "File and size") %>% 
  align(j = 1, align = "left", part = "body") %>% 
  align(j = 2, align = "right", part = "body") %>% 
  style(j = 1, pr_t = fp_text(color = "#58792d", underline = TRUE), part = "body") %>% 
  border_outer(border = fp_border()) %>%
  border_inner(border = fp_border()) %>%
  autofit(add_w = 0, add_h = 0.06) %>% 
  width(j = 1, width = 13.85 / 2.54) %>% 
  width(j = 2, width = 3.5 / 2.54)
```

\newpage

<Div custom-style = "Heading 2_nopgbrk">Contact</div>
**Name, title**  
Service  
Phone:  
Email:  
...
# Need to fix row heights in this table - hopefully fixed in next flextable update
flextable(MetaData, theme_fun = NULL) %>% 
  ...
  border_inner(border = fp_border()) %>%
  autofit(add_w = 0, add_h = 0.06) %>% 
  width(j = 1, width = 5.19 / 2.54) %>% 
  width(j = 2, width = 12.44 / 2.54)
```
